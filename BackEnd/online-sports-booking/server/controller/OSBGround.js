...
var OSBGround = require('../model/OSBGround');
const loginUser = require('./loginUserDetails')


router.get('/', (req, res) => {
    if (req.headers.authorization === process.env.Authorization) {
        OSBGround.find({}, function (err, data) {
            if (!err) {
...
router.post('/filter', (req, res) => {
    if (req.headers.authorization === process.env.Authorization) {
        ...
        OSBGround.find(filter, function (err, data) {
            if (!err) {
                res.send(JSON.stringify(data))
            }
            else {
...
router.get('/:id', (req, res) => {
    if (req.headers.authorization === process.env.Authorization) {
        OSBGround.find({ _id: req.params.id }, function (err, data) {
            if (!err) {
                res.send(JSON.stringify(data))
            }
        })
...
router.post('/', async (req, res) => {
    ...
    if (loginDetails._id) {
        if (req.headers.authorization === process.env.Authorization) {
            var obj = new OSBGround();
            obj.name = req.body.name;
            obj.capacity = req.body.capacity;
            obj.suitedFor = req.body.suitedFor;
...
router.put('/:id', async (req, res) => {
    ...
    if (loginDetails._id) {
        if (req.headers.authorization === process.env.Authorization) {
            ...
            req.body.capacity ? obj.capacity = req.body.capacity : false;
            req.body.suitedFor ? obj.suitedFor = req.body.suitedFor : false;
            obj.ModifiedBy = loginDetails;
            obj.Modified = new Date();
            OSBGround.findByIdAndUpdate(req.params.id,
...
router.delete('/:id', (req, res) => {
    if (req.headers.authorization === process.env.Authorization) {
        OSBGround.findByIdAndDelete((req.params.id),
            function (err, data) {
                if (err) {
                    let output = { isSuccess: false, message: err.toString() }
                    res.send(JSON.stringify(output));
